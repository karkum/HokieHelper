...
public class Maps_DrivingDirectionsGoogleKML extends Maps_DrivingDirections
{
	@Override
	...
	private class LoadDirectionsTask extends AsyncTask<Mode, Void, Maps_RouteImpl>
	{
		...
		@Override
		protected Maps_RouteImpl doInBackground(Mode... params)
		{
		...
		private Maps_RouteImpl parseResponse(InputStream inputStream) throws Exception
		{
			...
			//
			List<Maps_Placemark> placemarks = new ArrayList<Maps_Placemark>();
			for (int i = 0; i < placemarkList.getLength(); i++)
			{
				Maps_PlacemarkImpl placemark = parsePlacemark (placemarkList.item(i));
				if (placemark != null) {
			...
			//
			Maps_RouteImpl route = parseRoute (placemarkList);
			route.setPlacemarks(placemarks);
			
			return route;
		}

		private Maps_PlacemarkImpl parsePlacemark(Node item)
		{
			Maps_PlacemarkImpl placemark = new Maps_PlacemarkImpl ();
		...
		private Maps_RouteImpl parseRoute(NodeList placemarkList)
		{
			Maps_RouteImpl route = null;
		...
		private Maps_RouteImpl parseRoute(Node item)
		{
			Maps_RouteImpl route = new Maps_RouteImpl ();
		...
		protected void onPostExecute (Maps_RouteImpl route)
		{
			if (route == null) {
				Maps_DrivingDirectionsGoogleKML.this.onDirectionsNotAvailable();
			}
			else {
				Maps_DrivingDirectionsGoogleKML.this.onDirectionsAvailable(route);
			}
		}
	}
}
