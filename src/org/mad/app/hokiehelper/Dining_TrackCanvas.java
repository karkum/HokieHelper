...
public class Dining_TrackCanvas extends View 
{
    ...
    private void weekly(Canvas canvas)
    {
    	...
    	for (int i = 0; i < 7; i++)
        {
        	if (Dining_HallDietChooserActivity.calValues[29 - i].getCal() > 0)
        		sum += Dining_HallDietChooserActivity.calValues[29 - i].getCal();
        }
    	...
    	else
    	{
    		...
    		{
    			int cal = Dining_HallDietChooserActivity.calValues[29 - i].getCal();
    			int temp = cal;
    			if (temp != -1 && temp < 3000)
    			{
    				...
    				String text = Dining_HallDietChooserActivity.calValues[29 - i].getDate().toString().split("\\s+")[0];
